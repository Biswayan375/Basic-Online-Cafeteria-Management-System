...
<title>The Cafe Shop - My Orders</title>
...
<body style="background-image: url('Static/images/background.jpeg');">
	...
	<div style="margin-top: 10px; background-color: whitesmoke" class="container">
		<h2 class="display-3">My <font color="green">Orders</font></h2>
		<hr>
	</div>
	...
	<%
		if (session.getAttribute("flash-my-orders-error-message") != null) {
			...
			<div style="width: 790px; margin: auto; margin-bottom: 28px;" class="alert alert-danger alert-dismissible fade show" role="alert">
			  ...
			  <%= session.getAttribute("flash-my-orders-error-message") %>
			  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
			</div>
			<%
			session.removeAttribute("flash-my-orders-error-message");
		} else if (session.getAttribute("flash-my-orders-success-message") != null) {
			%>
			<div style="width: 790px; margin: auto; margin-bottom: 28px;" class="alert alert-success alert-dismissible fade show" role="alert">
			  ...
			  <%= session.getAttribute("flash-my-orders-success-message") %>
			  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
			</div>
			<%
			session.removeAttribute("flash-my-orders-success-message");
		}
	%>
	<!-- 
		--------------------------------------------------------------------------------------------------------------------
	...
	<section class="box-section">
		<div style="max-height: 450px; height: min-content; overflow: auto; width: 45%; background-color: white" class="shadowed-form">
			<h1 style="color: #6dae25;" class="display-5">Order History</h1>
			<hr>
			<jsp:include page="../components/previous-all-orders-component.jsp" />
		</div>
		
		<div style="max-height: 450px; height: min-content; overflow: auto; width: 45%; background-color: white" class="shadowed-form">
			<h1 style="color: #386b00;" class="display-5">Current Orders</h1>
			<hr>
			<jsp:include page="../components/current-all-orders-component.jsp" />
		</div>
	</section>
