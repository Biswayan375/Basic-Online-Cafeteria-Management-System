...
<body>
	<%
		if (session.getAttribute("flash-my-orders-error-message") != null) {
		...
		} else if (session.getAttribute("flash-my-orders") != null) {
			ResultSetAndStatement result = (ResultSetAndStatement)session.getAttribute("flash-my-orders");
			result.rs.last();
			...
			if (n > 0) {
				...
				%>
				<span style="display: block; text-align: center; color: blue; font-weight: bold;">You Currently have no PENDING records :)</span>
				<%
			}
			result.doneWithIt();
			session.removeAttribute("flash-my-orders");
		}
	%>
